...
def inference():
    ...
    model = PeftModel.from_pretrained(model, peft_model_id, cache_dir=model_args.cache_dir)
    model = model.cuda()
    model.eval()
    ...
    ## 新建输出路径
    os.makedirs(os.path.join(generating_args.output_path, model_args.model_name_or_path, 'from_english'), exist_ok=True)
    os.makedirs(os.path.join(generating_args.output_path, model_args.model_name_or_path, 'to_english'), exist_ok=True)
    ...
    # nllb 支持的语言对 list
    nllb_tokenizer = transformers.AutoTokenizer.from_pretrained(
        'facebook/nllb-200-3.3B',
        cache_dir=model_args.cache_dir
    )
    all_support_list = nllb_tokenizer.additional_special_tokens
    problem_lang_list = ['sat_Beng']
    rest_use_list = list(set(all_support_list).difference(set(problem_lang_list)))
    print(rest_use_list)

    ### 数据处理
    ## 循环
    for lang in rest_use_list:
        print('generate the translation for ' + lang + '.')
        ## load dataset from list
        src_data_list = []
        if generating_args.from_english:
            src_lang = 'eng_Latn'
            tgt_lang = lang
        else:
            src_lang = lang
        ...
        if generating_args.from_english:
            output_file = open(os.path.join(generating_args.output_path, model_args.model_name_or_path, 'from_english', src_lang + '-' + tgt_lang + '.jsonl'), 'w')
        else:
            output_file = open(os.path.join(generating_args.output_path, model_args.model_name_or_path, 'to_english', src_lang + '-' + tgt_lang + '.jsonl'), 'w')
