...
def inference():
    ...
    def evaluate_by_generate(
        ...
        # _INSTRUCTION = 'Sentence 1: {sentence1} Sentence 2: {sentence2} Question: Does Sentence 1 paraphrase Sentence 2? Please return either yes ot no.'
        _INSTRUCTION = "The following are two sentences:\nSentence1: {sens1}\nSentence2: {sens2}.\nYou should provide a yes or no answer to the question: Does Sentence1 paraphrase Sentence2?\nPlease only return either yes or no."

        prompt = [generate_prompt(instruction=_INSTRUCTION.format(sens1=sens1, sens2=sens2), template=template) for sens1, sens2 in zip(dataset["sentence1"], dataset["sentence2"])]
    ...
    ## 新建输出路径
    os.makedirs(os.path.join(generating_args.output_path, model_args.model_name_or_path, 'PAWS-X'), exist_ok=True)

    lang_list = ['de', 'en', 'es', 'fr', 'ja', 'ko', 'zh']
    ...
    for lang in lang_list:
        ...
        dataset = load_dataset('csv', data_files=os.path.join(data_args.data_path, lang, 'test_2k.tsv'), delimiter='\t')

        output_file = open(os.path.join(generating_args.output_path, model_args.model_name_or_path, 'PAWS-X',  lang + '.jsonl'), 'w')
