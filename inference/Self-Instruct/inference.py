...
_TRANSLATION_INSTRUCTION = "Translate the following sentence from {source_lang} to {target_lang}. Please only return the translated {target_lang} text."
...
class DataArguments:
    ...
    dataset_name: str = field(default=None, metadata={"help": "Path to the training data."})
...
def inference():
    ...
    def evaluate_by_generate(
        ...
        prompt = [generate_prompt(instruction=ins, input=inp[0]['input'], template=template) for ins, inp in zip(dataset["instruction"], dataset['instances'])]
    ...
    ## 新建输出路径
    if data_args.dataset_name == 'self_instruct':
        os.makedirs(os.path.join(generating_args.output_path, model_args.model_name_or_path, data_args.dataset_name), exist_ok=True)
        output_file = open(os.path.join(generating_args.output_path, model_args.model_name_or_path, data_args.dataset_name, 'predict.jsonl'), 'w')
        ### 数据处理
